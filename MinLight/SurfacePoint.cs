...
namespace MinLight
{
    ...
    */
    public class SurfacePoint
    {
        ...
        public bool GetNextDirection(Sampler random, Vector inDirection, out Vector outDirection, out Vector color)
        {

            float reflectivityMean = Item.Reflectivity.Dot(Vector.ONE) / 3.0f;

            // russian-roulette for reflectance magnitude
            if (random.GetNextSample() < reflectivityMean)
            {
                color = Item.Reflectivity / reflectivityMean;

                // cosine-weighted importance sample hemisphere

                double _2pr1 = Math.PI * 2.0f * random.GetNextSample();
                double sr2 = Math.Sqrt(random.GetNextSample());
